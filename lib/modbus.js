...
class ModbusConnection {
     ...
     * @returns {Promise<boolean>} Resolves to true if connection succeeded, false otherwise
     */
    async connect() {
            ...
            if (this.config.connectionType === "tcp") {
                ...
                await Promise.race([
                    ...
                    new Promise((_, reject) =>
                        setTimeout(() => reject(new Error(`TCP connection timeout after ${connectTimeout}ms`)), connectTimeout)
                    )
                ]);
            } else {
                // Serial RTU
     ...
     * @returns {Promise<number[]>} Resolves with an array of register values
     */
    async readInputRegisters(slaveId, address, qty) {
        ...
        const result = await this.client.readInputRegisters(address, qty);
        return result.data;
    }
    ...
    async readHoldingRegisters(slaveId, address, qty) {
        ...
        this.client.setID(slaveId);
        const result = await this.client.readHoldingRegisters(address, qty);
        return result.data;
    }
     ...
     * @returns {Promise<void>} Resolves when the register has been written
     */
     ...
     * @returns {Promise<void>} Resolves when all registers have been written
     */
     ...
     * @returns {Promise<{success: boolean, message: string}>} Result object with success flag and human-readable message
     */
     ...
     * @returns {number|string} Parsed and optionally scaled register value; string for STRING type
     */
