...
class ModbusConnection {
	...
	async readInputRegisters(slaveId, address, qty) {
		...
		this.log.debug(`FC04 slaveId=${slaveId} addr=${address} → [${data.slice(0, 4).join(',')}${data.length > 4 ? '…' : ''}]`);
		return data;
	}
	...
	async readHoldingRegisters(slaveId, address, qty) {
		...
		this.log.debug(`FC03 slaveId=${slaveId} addr=${address} → [${data.slice(0, 4).join(',')}${data.length > 4 ? '…' : ''}]`);
		return data;
	}
