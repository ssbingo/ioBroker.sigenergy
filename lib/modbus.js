...
// modbus-serial v8 ships CommonJS class export without construct signature
// in its type definitions — cast once at the require boundary.
// modbus-serial v8 ships its CommonJS class export without a construct
// signature in the bundled type definitions — restore it via the global
// ModbusRTUConstructor type (see lib/adapter-config.d.ts).
const ModbusRTU = /** @type {ModbusRTUConstructor} */ (/** @type {unknown} */ (require('modbus-serial')));
...
class ModbusConnection {
     ...
     * @param {object} logFn - Logging object (adapter.log compatible: debug/info/warn/error)
     */
    constructor(config, logFn) {
        ...
        // Deduplication key for repeated connection errors — prevents Sentry / log flooding
        // when the device is unreachable for an extended period.
    ...
    async connect() {
        ...
        try {
            ...
            this.connected = true;
            this.connecting = false;
            // Reset deduplication key after a successful connect so the next failure
            // is logged as ERROR again (one alert per outage, not per retry).
            if (this._lastErrorKey !== null) {
                this.log.info('Modbus connection recovered');
                this._lastErrorKey = null;
            }
            ...
            this.connected = false;
            this.connecting = false;
            // Close any half-open socket from a lost Promise.race — prevents the
            // "ghost socket" leak when connectTCP eventually resolves after our timeout.
            try {
            ...
            // Deduplicate: log first occurrence as ERROR, subsequent identical failures as DEBUG.
            const key = `connect:${err.code || err.message}`;
            if (this._lastErrorKey !== key) {
                this.log.error(`Modbus connection failed: ${err.message}`);
                this._lastErrorKey = key;
            } else {
                this.log.debug(`Modbus connection still failing: ${err.message}`);
            }
            return false;
        }
    }
    ...
    /**
     * Encode a value into raw 16-bit registers based on data type
     *
     * @param {number} value - Value to encode (engineering units)
     * @param {string} type - Data type (U16, S16, U32, S32)
     * @param {number|null} gain - Scaling factor
     * @returns {number[]} Raw register words
     */
