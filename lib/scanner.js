...
class SigenMicroScanner {
     ...
     * @param {object} log    - Logger {debug, info, warn, error, isStopped?}
     */
    constructor(config, log) {
        this.config = config;
        this.log = log || console;
        this._setTimeout = log && log.setTimeout ? log.setTimeout.bind(log) : setTimeout;
        // Optional shutdown-state getter. If set by the caller, the scan loop checks
        // it on every iteration so the scan can abort immediately on adapter unload
        // instead of blocking shutdown for minutes.
        this._isStopped = log && typeof log.isStopped === 'function' ? log.isStopped : () => false;
    }
     ...
     * @param {(text: string) => void | Promise<void>} [progressCb] - Progress callback
    ...
    async scan(from, to, modbus, progressCb) {
        ...
        this.log.debug(`[SigenMicroScanner] timeout per ID: 1000ms, pause between IDs: 80ms`);
        ...
        try {
            for (let id = from; id <= to; id++) {
                // Bail out quickly if the adapter is shutting down — a long-range
                // scan otherwise keeps running for several minutes after onUnload.
                if (this._isStopped()) {
                    this.log.info('[SigenMicroScanner] Scan aborted — adapter is shutting down');
                    break;
                }
                probed++;
                ...
                try {
                    ...
                    const model = String(ModbusConnection.parseValue(modelRaw, 'STRING', null) || '');
                    ...
                    try {
                        ...
                        serial = String(ModbusConnection.parseValue(serialRaw, 'STRING', null) || '');
                    } catch {
                        // serial is optional
                    }
                ...
                await new Promise(r => this._setTimeout(r, 80));
            }
        } finally {
