...
class Sigenergy extends utils.Adapter {
    /**
     * @param {Partial<utils.AdapterOptions>} [options] - Adapter options passed to the ioBroker core
     */
    constructor(options) {
        super({
            ...options,
     ...
     * @param {object[]} registers - Array of register definitions to group
     * @param {number} maxQty - Maximum number of registers allowed per read request
     * @returns {object[]} Array of grouped register read requests
     */
     ...
     * @param {object} group - A read group as produced by _buildReadGroups
     * @param {number[]} rawData - Raw register data returned by the Modbus read
     * @param {string} _section - Logical section name for logging (unused, reserved for future use)
     */
     ...
     * @param {string} name - State ID of the register (e.g. "plant.essPower")
     * @param {number|string} value - Parsed value of the register
     */
     ...
     * @param {string} id - Full state ID that changed
     ...
     * @param {ioBroker.Message} obj - Message object sent from admin or other adapters
     */
    async onMessage(obj) {
    ...
    /**
     * Adapter shutdown
     * @param {() => void} callback - Must be called after cleanup to signal shutdown completion
     */
