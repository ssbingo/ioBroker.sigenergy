...
### 3.0.10 (2026-06-29)
- (ssbingo) chore: js-yaml 从 4.1.1 升级到 4.3.0
...
### 3.0.4 (2026-06-14)
- (ssbingo) 修复：电网断电时 Telegram 通知现在只发送一次（不再每次轮询重复发送）；离网期间设备切换最多执行 3 次（初始 + 2 次重试）

### 3.0.3 (2026-06-13)
- (ssbingo) 修复：从 io-package.json 中删除无效的 welcomeText；在紧急断电配置选项卡中添加可见警告 staticText（黄色提示框，支持全部 11 种语言 i18n）
